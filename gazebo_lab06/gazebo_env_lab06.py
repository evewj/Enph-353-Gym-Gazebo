...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        hsv_frame = cv2.cvtColor(cv_image, 40)

        height, width, channels = cv_image.shape
        num_divisions = len(state)
        pixels_per_division = width / num_divisions

        #Finds centroid of line
        mask = cv2.inRange(hsv_frame, (path_hue-hue_match_tolerance,0,0), (path_hue + hue_match_tolerance,255,255))
        M = cv2.moments(mask)
        ...
        #cv_image = cv2.putText(cv_image, str(state), (0, 3*height/4), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
